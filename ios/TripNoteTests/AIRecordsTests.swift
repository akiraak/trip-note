...
struct AIRecordsTests {
    ...
    @Test func 生成ジョブの状態応答をデコードする() throws {
        ...
        let running = try JSONDecoder().decode(
            AIJobStatusResponse<AITripOutlineSuggestion>.self,
        ...
        let succeeded = try JSONDecoder().decode(
            ...
            { "id": "a", "status": "succeeded", "error": null,
              "result": { "destinationLatitude": null, "destinationLongitude": null,
                "candidates": [ { "dayCount": 2, "title": "1泊2日", "nights": [] } ] } }
            """.utf8)
        )
        #expect(succeeded.result?.candidates.map(\.title) == ["1泊2日"])

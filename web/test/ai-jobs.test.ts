...
describe("createAiJob", () => {
  it("trip_outline ジョブを pending で登録する", () => {
    ...
    expect(job.input).toMatchObject({ ...OUTLINE_INPUT, transport: null });
  });

  it("同 id の再送は既存ジョブをそのまま返す(冪等)", () => {
    createAiJob({ id: JOB_ID, kind: "trip_outline", input: OUTLINE_INPUT });
    getDb()
      .prepare("update ai_jobs set status = 'succeeded' where id = ?")
      .run(JOB_ID);
    const again = createAiJob({ id: JOB_ID, kind: "trip_outline", input: OUTLINE_INPUT });
  ...
  it("UUID でない id / 未知の kind / 不正な入力は拒否する", () => {
    expect(() =>
      createAiJob({ id: "abc", kind: "trip_outline", input: OUTLINE_INPUT }),
    ).toThrow(/UUID/);
    expect(() =>
      createAiJob({ id: JOB_ID, kind: "search_assist", input: {} }),
    ).toThrow(/kind/);
    expect(() =>
      createAiJob({ id: JOB_ID, kind: "trip_outline", input: { destination: " " } }),
    ).toThrow();
    // 失敗時は行を作らない
...
describe("runAiJob", () => {
  it("成功したら succeeded + result を保存する", async () => {
    ...
    await runAiJob(JOB_ID, async (kind, input) => {
      expect(kind).toBe("trip_outline");
      expect(input).toMatchObject(OUTLINE_INPUT);
      return { candidates: [] };
    });
  ...
  it("pending 以外は claim できず生成を実行しない(二重実行防止)", async () => {
    createAiJob({ id: JOB_ID, kind: "trip_outline", input: OUTLINE_INPUT });
    await runAiJob(JOB_ID, async () => ({ candidates: [] }));
    let called = false;
    await runAiJob(JOB_ID, async () => {
      called = true;
      return { candidates: [{ overwritten: true }] };
    });
    expect(called).toBe(false);
    expect(getAiJob(JOB_ID)!.result).toEqual({ candidates: [] });
  });

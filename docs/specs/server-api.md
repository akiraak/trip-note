...
**地図と写真を閲覧専用**で表示する(デザインは `docs/plans/share-page-redesign.md`)。

- 一番上に**旅行全体の地図**(全幅。記録トラック・プランの破線ルート・チェックポイントのピン。
  **写真マーカーは載せない**)。旅行名と「期間 · 日数 · 写真の枚数」を地図の上に重ねる
- その下に**日ごとのカード**(その日の地図・立ち寄り先を繋いだ 1 行・その日の写真)。
  写真はタップで拡大し、旅行全体を撮影順に前後へ送れる
- 写真・動画は撮影時刻を表示 TZ の日付にして `trip_days.date` と突き合わせて日に配る。
  **どの日にも当たらないものは「その他の写真」として最後に出す**(黙って落とさない)
- 出さないもの: 旅行の情報の一覧(開始・終了・出発予定・目的地・地点数・総距離)、
  チェックポイントの種別・予定時刻・到着予想・座標未設定、日のタイトルとメモ、編集・削除・
  AI 提案・旅行の終了の導線
